...
- **Регресс на N+1** (`tests/test_api.py`). `django_assert_num_queries(2)` на списке вакансий:
  если кто-то уберёт `select_related`, тест покраснеет сразу, а не после жалоб на медленный API.
